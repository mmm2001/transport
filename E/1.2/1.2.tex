...
\section{Методы построения схем для численного решения уравнения переноса}

Будем рассматривать уравнение переноса в некоторой области:

\begin{equation*}
\frac{\partial u}{\partial t} + с \frac{\partial u}{\partial x} = 0
\end{equation*}
...
Конечно-разностный метод состоит в замене дифференциальных операторов $\frac{\partial }{\partial x}$, $\frac{\partial }{\partial t}$ разностными отношениями.

\subsubsection*{Схема "явный правый уголок": метод конечных разностей}

Рассмотрим шаблон схемы "явный правый уголок" (точки $(m-1,n)$, $(m,n)$, $(m,n+1)$).
Аппроксимируем дифференциальные операторы правыми производными: 

\begin{equation*}
\frac{\partial u}{\partial x} \approx \frac{y_{m+1}^n-y_m^n}{h}
\end{equation*}

\begin{equation*}
\frac{\partial u}{\partial t} \approx \frac{y_{m}^{n+1}-y_m^n}{\tau}
\end{equation*}
...
\subsubsection*{Схема "явный правый уголок": метод прямых}

Рассмотрим шаблон схемы "явный правый уголок" (точки $(m-1,n)$, $(m,n)$, $(m,n+1)$). Тогда,
\begin{equation*}
